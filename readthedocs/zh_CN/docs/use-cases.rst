...
每个人都有很多不同的身份，然而目前应用服务方大多以唯一标记为用户提供服务，比如：手机号、身份证ID等，用户以不同的身份使用应用服务需要提供多个唯一标记。比如：某个用户既是一名运动员，也是一位大学教授，该用户想以不同身份使用应用服务时，必须在应用上注册两个唯一性账号。对于用户而言，多重身份切换使用的门槛比较高。通过WeIdentity可以实现单个个体绑定多重身份（相当于VC），切换不同身份使用应用服务时只需要提供对应身份的VC即可，主体仍然不变，因为主体是独一无二的（相当于DID标识符）。
...
#. 在身份证明机构、运动员认证机构、大学教授认证机构、应用方之间搭建区块链网络，各机构和应用方作为节点接入并注册WeIdentity DID
#. 由身份证明机构为用户进行KYC并生成WeIdentity DID
#. 用户分别向运动员认证机构和大学教授认证机构申请运动员凭证和大学教授凭证
#. 运动员认证机构和大学教授认证机构验证用户DID并生成对应的凭证，发送给认证申请人，同时将凭证上链；
#. 用户使用不同身份登录应用，出具不同的可验证凭证提供给应用方
#. 应用方验证用户提供的凭证，完成身份验证登录
